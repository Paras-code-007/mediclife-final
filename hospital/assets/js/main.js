(function() {
  ...
  function initLeafletMap() {
    if ($('#leaflet-map').length) {
      const mapboxAccessToken = 'pk.eyJ1IjoidHlwZTBjb2RlIiwiYSI6ImNrdmNxejgxcjJsYTQydG9rbjdxZzBrdGoifQ.L35XjKdGNpJDk7cGS6UIaA';

      var map = L.map('leaflet-map').setView([37.8, -96], 4);
  ...
  function settings() {
    ...
    if (modal.length) {
      if (!getCookie('MAClosedSettings') || getCookie('MAClosedSettings') === 'false') {
        setTimeout(() => {
          modal.modal('show');
        }, 500);
      }
      ...
      modal.on('shown.bs.modal', function() {
        document.cookie = 'MAClosedSettings=false';
      });
      modal.on('hidden.bs.modal', function() {
        document.cookie = 'MAClosedSettings=true';
      });
    }
